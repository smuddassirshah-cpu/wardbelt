...
// clamped to TOAST_MAX_MS and cleared on unmount; the caller removes the toast in onExpire.
...
export function Toast({ message, durationMs, onExpire, onUndo, inline = false }: ToastProps) {
  useEffect(() => {
    const id = setTimeout(onExpire, Math.min(Math.max(0, durationMs), TOAST_MAX_MS));
    return () => {
      clearTimeout(id);
    };
  }, [durationMs, onExpire]);
